    ...
    <div className="space-y-10">
      ...
      <div className="sticky bottom-0 flex items-center justify-between border-t border-border bg-background/95 py-4 backdrop-blur">
        ...
        <button
          onClick={() => void save()}
          disabled={saving}
          className="cinematic-button inline-flex items-center gap-3 px-6 py-3 text-[10px] uppercase tracking-[0.24em] disabled:opacity-60"
        >
...
function ImagePreview({ src, alt }: { src?: string | null; alt: string }) {
  return src ? (
    ...
    <div className="grid h-64 place-items-center border border-border bg-muted text-sm text-ink-soft">
      No image
    </div>
  );
}
